...
////////////////////////////////PROBLEM 6///////////////////////////////////////
            ...
            do {
            //START CODE//////////////////////////////
            //totNum to count all numbers. It starts at 6++. 7 is the first #
            //that is not in the Funny Sequence (first prime above 5)
            unsigned int totNum=6;
            //corNum is a counter of the numbers in the Funny Sequence. It
            //starts at 6 because there were 6 numbers in the sequence when
            //totNum is at 7. 
            unsigned int corNum=6;
            //primeA5 is meant to be a counter that is going to create 
            //all prime numbers, above 7, and below totNum
            unsigned int primA5=2;
            ...
            do  {
                totNum++;
                       ...
                       funChk=true;
                        ...
                        for (int insNum=6; insNum<=totNum; insNum++) {   
                        bool ckPrime = true;
                                //PRIME FOR LOOP
                                //Checks each number to see if it is a prime #
                                for (int primA5=2; primA5<=sqrt(insNum); primA5++) {
                                        if (insNum%primA5==0) {
                                        ckPrime = false;
                                        }
                                }
                                ...
                                //precise
                                int funCalc=(totNum/insNum);
                                float totNumF=static_cast<float>(totNum);
                                float insNumF=static_cast<float>(insNum);
                                //totNum/i to see if divisible 
                                float funCalcF=static_cast<float>(totNumF/insNumF);
                                //chkInt & chkFlo should be bools, but I could 
                                //not get it to work with bools. 
                                bool chkInt=true, chkFlo=true;
                                //Sets integers to floats, so all values 
                                //for totNum/i
                                        if ((funCalcF==1 ||        
                                        funCalcF==2 ||
                                        funCalcF==3 ||
                                        ...
                                        //To check modular function, must 
                                        //convert funCalcF back to
                                        if (fmod(funCalcF, 2)==0||
                                            fmod(funCalcF, 3)==0||
                                            fmod(funCalcF, 5)==0){
                                            chkInt=false;
                                        }
                                        if ((chkFlo==false)||(chkInt==false)){
                                        funChk=false;
                                        //This statement ends the DIVISIBILITY
                                        //FOR LOOP by setting i to be greater 
                                        //than totNum.
                                        insNum=totNum+1;
                                        }
                                }
                ...
                //Check funChk to see if its true. If its true, then adds 1 to
                //corNum count. 
                if (funChk==true) {
                    corNum++;   
                }                 
            } while (corNum<1475);                          //End COUNTING LOOP
            //Output the 1475 corNum that comes out of the loops
            cout<<totNum<<" is the "<<corNum<<"th # in the Funny Sequence!";
